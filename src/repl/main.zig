...
const Client = struct {
    ...
    fn create_lens(self: *Client, label: []const u8, source_label: []const u8, transform: []const u8) !void {
        var stream = self.stream orelse return error.NotConnected;
        var payload: [32 + 32 + 32]u8 = undefined;
        const label_bytes = label_payload(label);
        const source_bytes = label_payload(source_label);
        @memcpy(payload[0..32], label_bytes[0..32]);
        @memcpy(payload[32..64], source_bytes[0..32]);
        @memcpy(payload[64..96], transform[0..@min(transform.len, 32)]);
        try send_request(&stream, .create_lens, &payload);
        try expect_ok(&stream);
    }

    fn drop_lens(self: *Client, label: []const u8) !void {
        var stream = self.stream orelse return error.NotConnected;
        var payload = label_payload(label);
        try send_request(&stream, .drop_lens, &payload);
        try expect_ok(&stream);
    }

    fn query_lens(self: *Client, label: []const u8, timestamp: i64) !?f64 {
        var stream = self.stream orelse return error.NotConnected;
        var payload: [32 + 8]u8 = undefined;
        const label_bytes = label_payload(label);
        @memcpy(payload[0..32], label_bytes[0..32]);
        std.mem.writeInt(i64, payload[32..40], timestamp, .big);
        try send_request(&stream, .query_lens, &payload);
        const resp = try read_response(&stream);
        switch (resp.opcode) {
            .ok => {
                if (resp.payload.len == 1 and resp.payload[0] == 0) return null;
    ...
    fn compose_lens(self: *Client, label: []const u8, lens1: []const u8, lens2: []const u8) !void {
        var stream = self.stream orelse return error.NotConnected;
        var payload: [32 + 32 + 32]u8 = undefined;
        const label_bytes = label_payload(label);
        const lens1_bytes = label_payload(lens1);
        const lens2_bytes = label_payload(lens2);
        @memcpy(payload[0..32], label_bytes[0..32]);
        @memcpy(payload[32..64], lens1_bytes[0..32]);
        @memcpy(payload[64..96], lens2_bytes[0..32]);
        try send_request(&stream, .compose_lens, &payload);
        try expect_ok(&stream);
    }

    fn list_lenses(self: *Client) ![][]const u8 {
        var stream = self.stream orelse return error.NotConnected;
        try send_request(&stream, .list_lenses, &.{});
        const resp = try read_response(&stream);
        switch (resp.opcode) {
            .ok => {
                const count = resp.payload.len / 32;
                var result = try self.allocator.alloc([]const u8, count);
                for (0..count) |i| {
                    const start = i * 32;
                    const label = resp.payload[start .. start + 32];
                    var trimmed = label;
                    while (trimmed.len > 0 and trimmed[trimmed.len - 1] == 0) {
                        trimmed = trimmed[0 .. trimmed.len - 1];
                    }
                    result[i] = try self.allocator.dupe(u8, trimmed);
                }
                return result;
            },
            .err => return error.RemoteError,
...
pub fn main() !void {
    ...
    while (true) {
        ...
        if (std.mem.eql(u8, cmd, "transforms")) {
            std.debug.print(
                "identity\n" ++
        ...
        if (std.mem.eql(u8, cmd, "create_series") or std.mem.eql(u8, cmd, "create")) {
            const label = iter.next() orelse {
                std.debug.print("usage: create_series <label>\n", .{});
                continue;
            };
            client.create_series(label) catch |err| {
                std.debug.print("create_series failed: {s}\n", .{@errorName(err)});
                continue;
            };
            std.debug.print("ok\n", .{});
            continue;
        ...
        if (std.mem.eql(u8, cmd, "drop_series") or std.mem.eql(u8, cmd, "drop")) {
            const label = iter.next() orelse {
                std.debug.print("usage: drop_series <label>\n", .{});
                continue;
            };
            client.drop_series(label) catch |err| {
                std.debug.print("drop_series failed: {s}\n", .{@errorName(err)});
                continue;
            };
            std.debug.print("ok\n", .{});
            continue;
        ...
        if (std.mem.eql(u8, cmd, "append_point") or std.mem.eql(u8, cmd, "append")) {
            const label = iter.next() orelse {
                std.debug.print("usage: append_point <label> <timestamp> <value>\n", .{});
                continue;
            };
            ...
            client.append_point(label, timestamp, value) catch |err| {
                std.debug.print("append_point failed: {s}\n", .{@errorName(err)});
                continue;
            };
            std.debug.print("ok\n", .{});
            continue;
        ...
        if (std.mem.eql(u8, cmd, "query_point") or std.mem.eql(u8, cmd, "query")) {
            const label = iter.next() orelse {
                std.debug.print("usage: query_point <label> <timestamp>\n", .{});
                continue;
            };
            const ts_str = iter.next() orelse {
                std.debug.print("usage: query_point <label> <timestamp>\n", .{});
            ...
            const result = client.query_point(label, timestamp) catch |err| {
                std.debug.print("query_point failed: {s}\n", .{@errorName(err)});
                continue;
            };
            if (result) |value| {
        ...
        if (std.mem.eql(u8, cmd, "create_lens")) {
            const label = iter.next() orelse {
                std.debug.print("usage: create_lens <label> <source> <transform>\n", .{});
                continue;
            };
            const source = iter.next() orelse {
                std.debug.print("usage: create_lens <label> <source> <transform>\n", .{});
                continue;
            };
            const transform = iter.next() orelse {
                std.debug.print("usage: create_lens <label> <source> <transform>\n", .{});
                continue;
            };
            client.create_lens(label, source, transform) catch |err| {
                std.debug.print("create_lens failed: {s}\n", .{@errorName(err)});
                continue;
            };
            std.debug.print("ok\n", .{});
            continue;
        ...
        if (std.mem.eql(u8, cmd, "drop_lens")) {
            const label = iter.next() orelse {
                std.debug.print("usage: drop_lens <label>\n", .{});
                continue;
            };
            client.drop_lens(label) catch |err| {
                std.debug.print("drop_lens failed: {s}\n", .{@errorName(err)});
                continue;
            };
            std.debug.print("ok\n", .{});
            continue;
        ...
        if (std.mem.eql(u8, cmd, "query_lens")) {
            const label = iter.next() orelse {
                std.debug.print("usage: query_lens <label> <timestamp>\n", .{});
                continue;
            };
            const ts_str = iter.next() orelse {
                std.debug.print("usage: query_lens <label> <timestamp>\n", .{});
            ...
            const result = client.query_lens(label, timestamp) catch |err| {
                std.debug.print("query_lens failed: {s}\n", .{@errorName(err)});
                continue;
            };
            if (result) |value| {
                std.debug.print("value: {d}\n", .{value});
        ...
        if (std.mem.eql(u8, cmd, "compose_lens")) {
            const label = iter.next() orelse {
                std.debug.print("usage: compose_lens <new_label> <lens1> <lens2>\n", .{});
                continue;
            };
            const lens1 = iter.next() orelse {
                std.debug.print("usage: compose_lens <new_label> <lens1> <lens2>\n", .{});
                continue;
            };
            const lens2 = iter.next() orelse {
                std.debug.print("usage: compose_lens <new_label> <lens1> <lens2>\n", .{});
                continue;
            };
            client.compose_lens(label, lens1, lens2) catch |err| {
                std.debug.print("compose_lens failed: {s}\n", .{@errorName(err)});
                continue;
            };
            std.debug.print("ok\n", .{});
            continue;
        ...
        if (std.mem.eql(u8, cmd, "list_lenses")) {
            const lenses = client.list_lenses() catch |err| {
                std.debug.print("list_lenses failed: {s}\n", .{@errorName(err)});
                continue;
            };
            defer {
                for (lenses) |l| client.allocator.free(l);
                client.allocator.free(lenses);
            }
            if (lenses.len == 0) {
                std.debug.print("no lenses\n", .{});
            } else {
                for (lenses) |l| {
                    std.debug.print("  {s}\n", .{l});
                }
