...
pub const benchmark = struct {
    ...
    /// Parallel benchmark: number of concurrent series.
    pub const parallel_series_count: u32 = 8;

    /// Parallel benchmark: points appended per series.
...
comptime {
    ...
    assert(benchmark.parallel_series_count > 0);
    assert(benchmark.parallel_series_count <= 64);
    assert(benchmark.parallel_points_per_series > 0);
}
...
test "benchmark config is valid" {
    ...
    try std.testing.expect(benchmark.parallel_series_count > 0);
    try std.testing.expect(benchmark.parallel_series_count <= 64);
    try std.testing.expect(benchmark.parallel_points_per_series > 0);
}
